...
namespace Google.Protobuf.Reflection
{
    ...
    public class DescriptorsTest
    {
        ...
        {
            var cachingEnabled = typeof(FileDescriptor).GetField("extensionCachingEnabled", BindingFlags.NonPublic | BindingFlags.Static);
            ...
            try
            {
                ...
                var dependencies = Enumerable.Range(0, 4096)
                    .Select(i => FileDescriptor.FromGeneratedCode(
                        new FileDescriptorProto
                        {
                            Name = $"{prefix}_{i}.proto",
                            Dependency = { extensionDescriptor.Name }
                        }.ToByteArray(),
                        new[] { extensionDescriptor }, generatedInfo))
                    .ToArray();

                Parallel.For(0, dependencies.Length, i =>
                {
                    // Exercise both distinct cache misses and concurrent access to shared dependencies.
                    var fileDependencies = new[] { dependencies[i], dependencies[i / 2] }.Distinct().ToArray();
                    ...
                    proto.Options.SetExtension(UnittestCustomOptionsProto3Extensions.FileOpt1, (ulong) i + 1);
                    var descriptor = FileDescriptor.FromGeneratedCode(
                        proto.ToByteArray(), fileDependencies, generatedInfo);
                    CollectionAssert.AreEqual(fileDependencies, descriptor.Dependencies);
                    Assert.AreEqual((ulong) i + 1,
                        descriptor.GetOptions().GetExtension(UnittestCustomOptionsProto3Extensions.FileOpt1));
                });
                ...
                var unrelatedDependency = FileDescriptor.FromGeneratedCode(
                    ...
                    Array.Empty<FileDescriptor>(), generatedInfo);
                ...
                unrelatedProto.Options.SetExtension(UnittestCustomOptionsProto3Extensions.FileOpt1, 1UL);
                var unrelatedDescriptor = FileDescriptor.FromGeneratedCode(
                    unrelatedProto.ToByteArray(), new[] { unrelatedDependency }, generatedInfo);
                Assert.IsFalse(unrelatedDescriptor.GetOptions().HasExtension(UnittestCustomOptionsProto3Extensions.FileOpt1));
            }
            finally
            {
